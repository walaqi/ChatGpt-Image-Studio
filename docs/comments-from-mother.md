...
返回该用户**当前能出图**的 key 列表，供 image-studio 渲染选择器。**不含明文 key**：
...
按用户选定的 `key_id` 返回真正凭证，返回前**再校验一次**归属（key 属于该 uid）、未过期、有剩余额度、仍满足出图条件：
...
| D3 | 入口 ticket 签发端点 | ✅ | `GET /api/v1/image-studio/ticket`（已登录用户，走 `jwtAuth`）。RS256 签 `{sub,iss,aud,iat,exp,jti}`；`sub` = userID 的**字符串化 int64**；TTL = `ticket_ttl_seconds`（默认 60s）。返回 `{ "ticket": "<jwt>", "expires_at": <unix秒> }`。功能未启用时返回 404 `Image studio is not enabled`。 |
...
```
取明文 `GET /internal/cred?uid=&key_id=` 返回：
```json
{ "api_key": "sk-...", "base_url": "http://<网关内网地址>/v1", "model": "gpt-image-2" }
```
越权（A 用 B 的 key_id）或 key 已不满足出图条件 → **404**。
